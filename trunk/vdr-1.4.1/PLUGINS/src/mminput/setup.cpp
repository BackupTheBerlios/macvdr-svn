...
cMMInputSetup MMInputSetup;

cMMInputSetup::cMMInputSetup(void) {
	RemotePort    = 2004;
#if VDRVERSNUM >= 10300
	StreamFilters = false;
#endif
...
bool cMMInputSetup::SetupParse(const char *Name, const char *Value) {
	if (strcmp(Name, "SyncEPG") == 0)       SyncEPG = atoi(Value);
	else return false;
	return true;
}
...
cMMInputMenuSetupPage::cMMInputMenuSetupPage(void) {
	m_NewSetup = MMInputSetup;
	AddBoolEdit (tr("Synchronize EPG"),    m_NewSetup.SyncEPG);
	SetCurrent(Get(0));
}

cMMInputMenuSetupPage::~cMMInputMenuSetupPage() {
}

void cMMInputMenuSetupPage::AddCategory(const char *Title) {
  char *buffer = NULL;
...
void cMMInputMenuSetupPage::AddBoolEdit(const char *Title, int &Value) {
	Add(new cMenuEditBoolItem(Title, &Value));
}

void cMMInputMenuSetupPage::AddIpEdit(const char *Title, char *Value) {
	Add(new cMenuEditIpItem(Title, Value));
}

void cMMInputMenuSetupPage::AddShortEdit(const char *Title, int &Value) {
	AddRangeEdit(Title, Value, 0, 65535);
}

void cMMInputMenuSetupPage::AddRangeEdit(const char *Title, int &Value, 
		int Min, int Max) {
	Add(new cMenuEditIntItem(Title, &Value, Min, Max));
}

void cMMInputMenuSetupPage::AddSuspEdit(const char *Title, int &Value) {
...
void cMMInputMenuSetupPage::AddTypeEdit(const char *Title, int &Value) {
...
void cMMInputMenuSetupPage::Store(void) {
        ...
        MMInputSetup = m_NewSetup;

        cMMInputDevice::ReInit();
}
