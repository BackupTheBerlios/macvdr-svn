...
{
        //	printf("tComponent::FromString\n");
        unsigned int Stream, Type;
        unsigned int tmpSize = strlen(s);
        if(description == NULL){
                description = (char*)malloc(tmpSize);
                memset(description,0,tmpSize);
        }
        
        int n = sscanf(s, "%X %02X %7s %s[^\n]", &Stream, &Type, language, description); // 7 = MAXLANGCODE2 - 1
...
bool cComponents::Realloc(int Index){
        if (Index >= numComponents) {
                ...
                if(components == NULL){
                        //		fprintf(stderr,"cComponents::Realloc: reallocation of components failed malloc_size: %d\n",malloc_size(components));
                        return false;
                }
                else{
